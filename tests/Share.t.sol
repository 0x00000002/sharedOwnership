...
contract ShareTests is Test {
    ...
    function setupShareContract() internal {
        share_ = new Share(aManager);
        aShare = address(share_);

        bytes4[] memory selectors = new bytes4[](2);
        selectors[0] = share_.mint.selector;
        selectors[1] = share_.burn.selector;

        vm.startPrank(admin);
        am_.grantRole(SHARE_ADMIN, admin, 0);
        am_.setTargetFunctionRole(aShare, selectors, SHARE_ADMIN);

        (bool canCall, ) = am_.canCall(admin, aShare, share_.mint.selector);
