...
contract GameItems is ERC1155 /* Contract inherits from ERC1155 spec */ {
    ...
    uint256 public constant VESSEL_INITIAL_VALUE =
        10 ** 17; /* Initial value of 0.1 ETH (prices in wei) */

    uint256 public constant CRUISER_ID = 1;
    uint256 public constant CRUISER_COUNT = 100;
    uint256 public constant CRUISER_INITIAL_VALUE =
        10 ** 17; /* Initial value of 0.1 ETH */

    uint256 public constant DREADNOUGHT_ID = 2;
    uint256 public constant DREADNOUGHT_COUNT = 50;
    uint256 public constant DREADNOUGHT_INITIAL_VALUE =
        5 * (10 ** 17); /* Initial value of 0.5 ETH */

    uint256 public constant BATTLESHIP_ID = 3;
    uint256 public constant BATTLESHIP_COUNT = 5;
    uint256 public constant BATTLESHIP_INITIAL_VALUE =
        2 * (10 ** 18); /* Initial value of 2 ETH */

    uint256 public constant DESTROYER_ID = 4;
    uint256 public constant DESTROYER_COUNT = 1;
    uint256 public constant DESTROYER_INITIAL_VALUE =
        10 ** 19; /* Initial value of 10 ETH */

    uint256 public constant AUCTION_END_TIME =
        1611230400; /* epoch time (seconds) - Currently set to 01/21/2021 @ 12:00pm (UTC) */

    uint256 public constant MAX_ID =
        4; /* max id for a token class, range is 0 <= id <= MAX_ID */
    ...
    function distribute() public {
        ...
        owner.transfer(
            ...
            ); /* Send contract owner all funds in the smart contract */
    }
    ...
    function bid(uint256 id) public payable {
        require(
            id <= MAX_ID,
            "Id doesn't exist"
        ); /* Require that the sent item ID exists */
        require(
            msg.value >= subauctions[id].initial_price,
            "Sent value below initial price"
        );
        ...
        for (uint256 i = 0; i < subauctions[id].bidcount; i++) {
            /* only for count - 1 since we look at next value in loop */
            /* comparing to next bid since head value is checked before this */
            if (
                i == subauctions[id].bidcount - 1 ||
                ...
                uint256 oldhead = subauctions[id]
                    .head; /* save head for repayment of bid, since bidder gets kicked */

                /* for distributing one token, the head remains, pointing to same index (0) */
                if (subauctions[id].token_count > 1) {
                    subauctions[id].head = subauctions[id]
                        .bids[subauctions[id].head]
                        .next; /* set new head to next largest */
                }
                subauctions[id].bids[oldhead] = new_bid;

                /* send back money to address kicked off list */
                uint256 amount = subauctions[id].bids[current_bid.next].value -
                    2300 *
                    tx.gasprice;
                subauctions[id]
                    .bids[current_bid.next]
                    .value = 0; /* stop re-entrancy vulnerability */
